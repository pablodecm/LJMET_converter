...
class ConverterRunOne : public TSelector {
  public :
    ...
    std::unique_ptr<TTreeReaderValue<double>> event; 
    std::unique_ptr<TTreeReaderValue<int>> lumi;
    std::unique_ptr<TTreeReaderValue<int>> run; 
    std::unique_ptr<TTreeReaderValue<int>> nPV;
    std::unique_ptr<TTreeReaderValue<int>> goodLumi;
    ...
    std::unique_ptr<TTreeReaderValue<double>> pf_met_pt;
    std::unique_ptr<TTreeReaderValue<double>> pf_met_px;
    std::unique_ptr<TTreeReaderValue<double>> pf_met_py;
    ...
    ConverterRunOne(TTree * /*tree*/ =0) :
      event(new TTreeReaderValue<double>(reader,"event_mu")), 
      lumi(new TTreeReaderValue<int>(reader, "lumi_mu")),
      run(new TTreeReaderValue<int>(reader, "run_mu")),
      nPV(new TTreeReaderValue<int>(reader, "nPV_mu")),
      goodLumi(new TTreeReaderValue<int>(reader, "GoodLumi_mu")),
      pf_met_pt( new TTreeReaderValue<double>(reader,"PF_met_pt_mu")),
      pf_met_px( new TTreeReaderValue<double>(reader,"PF_met_px_mu")),
      pf_met_py( new TTreeReaderValue<double>(reader,"PF_met_py_mu"))
   {}
